/*
  ...
  https://github.com/cityverve/google-maps-demo
...
function initialise ()
{
    ...
    var map  = new google.maps.Map (document.getElementById ('map'), MANCHESTER);
    var curr = null;
    var url  = API_ENDPOINT + entity;

    map.addListener ('click', function ()
    {
        if (curr != null) curr.close ();  // close any previously opened info box
        curr = null;
    });
    ...
    httpGet (url, function (entities)
    {
        entities.forEach (function (entity)
        {
            httpGet (entity.uri, function (items)
            {
                ...
                {
                    var pos  = { lat: item.loc.coordinates [1], lng: item.loc.coordinates [0] };  // turns a geojson point into a google position
                    var pin  = new google.maps.Marker ({ position: pos, map: map, title: item.name });
                    var info = new google.maps.InfoWindow ({ content: infobox (item) });
